...
class queryFactory extends base {
  ...
  function queryFactory() {
    ...
    // End of Additional Universal Fields.
  }
...
//@TODO error class required to virtualise & centralise all error reporting/logging/debugging
  ...
  function show_error() {  	
    ...
    if(SHOW_MYSQL_ERROR == 'false'){
    	if($this->error_number != 1062){
	    	...
	    	error_log(date('Y-m-d H:i', time()) .' ' . $this->error_number . ' ' . $this->error_text . "\n", 3, MYSQL_ERROR_LOG);    	
	    	error_log($_SERVER["REQUEST_URI"] . "\n", 3, MYSQL_ERROR_LOG);
	    	error_log('[' . $this->zf_sql . ']'. "\n", 3, MYSQL_ERROR_LOG);    	
	    	error_log("\n\n");
	    	//echo "<script>window.location.href='".HTTP_SERVER . DIR_WS_CATALOG ."nddbc.html';</script>";
	    	return;
    	}
    }
  ...
  function Execute($zf_sql, $zf_limit = false, $zf_cache = false, $zf_cachetime=0) {
	...
	while (!$obj->EOF) {
      ...
      $zp_db_resource = @mysql_query($zf_sql, $this->link);
      if (!$zp_db_resource) {$this->set_error(@mysql_errno($this->link),@mysql_error($this->link)); throw new Exception('mysql error');}
      $obj->resource = $zp_db_resource;
      $obj->cursor = 0;
      if ($obj->RecordCount() > 0) {
        $obj->EOF = false;
  ...
  function get_table_fields($tableName) {
    if (!empty($this->table_fields[$tableName])) {
      return $this->table_fields[$tableName];
    }
    $this->table_fields[$tableName] = array();
    $result = @mysql_query("SHOW FIELDS FROM $tableName");
    while ($row = @mysql_fetch_array($result)) {
        if (isset($row['Default']) && !empty($row['Default'])) {
      $this->table_fields[$tableName][$row['Field']] = $row['Type'] . ' (' .$row['Default']. ')';      
        } else {
      $this->table_fields[$tableName][$row['Field']] = $row['Type'];      
        } // End of if statement.
    }
    return $this->table_fields[$tableName];
  }
